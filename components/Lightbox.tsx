...
export function Lightbox({ items, index, onClose, onChange }: Props) {
  ...
  return createPortal(<AnimatePresence>{active && index !== null && <motion.div data-lightbox role="dialog" aria-modal="true" aria-label={labels.dialog} className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 px-4 py-20" initial={reducedMotion ? false : {opacity:0}} animate={{opacity:1}} exit={reducedMotion ? undefined : {opacity:0}}>
