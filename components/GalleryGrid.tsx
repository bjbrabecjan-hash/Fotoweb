...
export type GalleryItem = Omit<WorkItem, "category"> & { category: GalleryCategory; titleEn?: string; altEn?: string };
  ...
  return <>
    ...
    <div className={featured ? "grid gap-4 sm:grid-cols-2 lg:grid-cols-3" : "masonry"}>
      {visibleItems.map((item,index) => {
        ...
        return <motion.button key={item.id} data-protected-media type="button" aria-label={`${locale === "cz" ? "Otevřít fotografii" : "Open photograph"}: ${title}`} className={cn("group relative mb-4 block w-full overflow-hidden bg-charcoal text-left focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-gold focus-visible:ring-offset-2", featured ? ratioClasses[item.ratio] : `${ratioClasses[item.ratio]} break-inside-avoid`)} initial={reducedMotion ? false : {opacity:0,y:18}} whileInView={reducedMotion ? undefined : {opacity:1,y:0}} viewport={{once:true,margin:"-80px"}} transition={{duration:.45,delay:Math.min(index*.03,.18)}} onClick={() => setActiveIndex(index)}>
          <Image src={item.src} alt={alt} fill sizes={featured ? "(min-width:1280px) 400px, (min-width:1024px) 31vw, (min-width:640px) 48vw, 100vw" : "(min-width:1280px) 400px, (min-width:1024px) 31vw, (min-width:640px) 48vw, 100vw"} className="object-cover transition-transform duration-700 group-hover:scale-[1.025]" />
          <span className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-80"/><span className="absolute inset-x-5 bottom-5"><span className="block text-[.62rem] uppercase tracking-luxe text-white/90">{t.portfolio.categories[item.category] ?? item.category}</span><span className="mt-1 block font-display text-2xl text-white">{title}</span></span>
        </motion.button>;
      })}
    </div>
